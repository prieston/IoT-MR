...
export const CalculateTransformation = (timeDelta, model) => {
  ...
  const action =
    window.iotmr.selected.object?.uuid == model.id
  ...
  const Xb = Number((currentPath[0] - window.iotmr.center[0]).toFixed(4));
  const Yb = Number((currentPath[2] - window.iotmr.center[2]).toFixed(4));
  ...
  const { Gab, Sab: TotalSab } = ThemeliodesProblima_2(
    start[0] - window.iotmr.center[0],
    start[2] - window.iotmr.center[2],
    Xb,
    Yb
  );
  if (
  ...
  if (currentAnimation.dynamicHeight) {
    ...
    newZ = start[1] - window.iotmr.center[1] + Zdif * percentage;
  }
